...
from pilotbook_mcp.sectors import bucket_direction, is_exposed
...
def score_anchorage(anchorage: Anchorage, forecast: list[dict]) -> Score:
    ...
    for step in forecast:
        wind_from = bucket_direction(step["wind_from_deg"])
        if is_exposed(anchorage.exposed_sectors, wind_from):
            w = step["wind_kn"] * WIND_WEIGHT
            if anchorage.holding == "poor":
                w *= POOR_HOLDING_MULT
            wind_penalty += w
            hit_sectors.add(wind_from)

        swell_deg = step.get("swell_from_deg")
        swell_m = step.get("swell_m")
        if swell_deg is not None and swell_m is not None:
            swell_from = bucket_direction(swell_deg)
            if is_exposed(anchorage.exposed_sectors, swell_from):
                swell_penalty += swell_m * SWELL_SCALE * SWELL_WEIGHT
                hit_sectors.add(swell_from)
